...
#define OUTPUT_AZT_CEA 0 //Safa _ Cost effectiveness analysis of AZT
...
struct input
{
  ...
  struct
  {
    ...
    double medication_costs[32]; //Safa : moved from medication+ 16 > 32
  ...
  struct
  {
    ...
    //Safa
    double medication_utility[32]; //Safa: moved from medicatio to costs + 16 > 32
    double hearing_dutil[4];
    double gis_dutil;
  } utility;
  ...
  struct
  {
    double medication_ln_hr_exac[32]; //Safa : changed from 16 to 32
...
void reset_output()
{
  ...
  output.azt_n_men=0;
  output.azt_cumul_time=0;    //End variable by nature;
  ...
  output.azt_total_gold[0]=0;output.azt_total_gold[1]=0;output.azt_total_gold[2]=0;output.azt_total_gold[3]=0;   //0:mild, 1:moderae, 2:severe; 3=very severe    END because agent records
  output.azt_exp_total_exacs=0;

  output.azt_total_cost=0;    //END because agent records
  output.azt_exp_total_cost=0;
  output.azt_total_qaly=0;  //END because agent records
  output.azt_exp_total_qaly=0;

  output.azt_total_cost_hearing=0;   //END because agent records
  output.azt_total_cost_gi=0;    //END because agent records

  output.azt_total_qaly_hearing=0;  //END because agent records
  output.azt_total_qaly_gi=0;  //END because agent records
...
double update_AZT(agent *ag) {  //if criteria is met, update medication!
  ...
  // if ((*ag).azt_eligible){
  //   (*ag).azt_flag = 0; //I'm usung if for sensitivity analysis: time_horizen == 1. when flag is 0 the agent would not be followed
  // }
  ...
  if(on_azt){
    ...
    if( calendar_time==0 && (*ag).gold > 1 && (*ag).azt_eligible  == 0 && (*ag).hearing_status == 0 && (*ag).diagnosis == 1
          && check_azt_eligibility(ag) && (*ag).local_time <=5) //  => I am  collecting my cohort from the first 5 years of Epic's simulation
...
(*ag).azt_flag = 1;
(*ag).azt_eligible  = 0;
(*ag).local_time_at_AZT = 0;
(*ag).azt_time_horizon = 0;
(*ag).hearing_status = 0;
(*ag).gastro_status = 0;
(*ag).cv_status = 0;
(*ag).azt_resistance = 1;
(*ag).time_at_hearing_loss = 0;
(*ag).notmild_exac_history_time_first=0;
(*ag).notmild_exac_history_time_second=0;
...
{
  //Safa changed feom 33 to 43
  NumericMatrix outm(event_stack_pointer,43);
  CharacterVector eventMatrixColNames(43);
...
// 'create' helper function is limited to 20 enteries
  ...
  //Safa #ifdef OUTPUT_AZT_CEA
  eventMatrixColNames(33) = "local_time_at_AZT";
  eventMatrixColNames(34) = "azt_eligible";
  eventMatrixColNames(35) = "hearing_status";
  eventMatrixColNames(36) = "gastro_status";
  eventMatrixColNames(37) = "cv_status";
  eventMatrixColNames(38) = "time_at_hearing_loss";
  eventMatrixColNames(39) = "cost_pre_azt";
  eventMatrixColNames(40) = "qaly_pre_azt";
  eventMatrixColNames(41) = "azt_time_horizon";
  eventMatrixColNames(42) = "azt_resistance";
  // #endif
  ...
  for(int i=0;i<event_stack_pointer;i++)
  {
    ...
    //Safa #ifdef OUTPUT_AZT_CEA
    outm(i,33)=(*ag).local_time_at_AZT;
    outm(i,34)=(*ag).azt_eligible ;
    outm(i,35)=(*ag).hearing_status;
    outm(i,36)=(*ag).gastro_status;
    outm(i,37)=(*ag).cv_status;
    outm(i,38)=(*ag).time_at_hearing_loss;
    outm(i,39)=(*ag).cost_pre_azt;
    outm(i,40)=(*ag).qaly_pre_azt;
    outm(i,41)=(*ag).azt_time_horizon;
    outm(i,42)=(*ag).azt_resistance;
  }
  // #endif
...
void event_exacerbation_process(agent *ag)
{
  ...
  //Safa: maybe you can replace these with the previous ones because no one needs not-mild previous exacerbation anyway
  if((*ag).exac_status > 1)
  {
    (*ag).notmild_exac_history_time_second=(*ag).notmild_exac_history_time_first;
    (*ag).notmild_exac_history_severity_second=(*ag).notmild_exac_history_severity_first;
    (*ag).notmild_exac_history_time_first=(*ag).local_time;
    (*ag).notmild_exac_history_severity_first=(*ag).exac_status;
  }
...
//Safa : changed while condition
    ...
    while(
      (((calendar_time+(*ag).local_time)<input.global_parameters.time_horizon && !(*ag).azt_eligible)
         ...
         || ((*ag).azt_eligible && (*ag).azt_flag && calendar_time==0 &&((calendar_time+(*ag).local_time)<(*ag).azt_time_horizon))
      )
...
//Safa : changed the condition
      // if(calendar_time+(*ag).local_time+tte<input.global_parameters.time_horizon)
      // {

      if(((calendar_time+(*ag).local_time+tte-input.global_parameters.time_horizon) < 0 && !(*ag).azt_eligible) ||
