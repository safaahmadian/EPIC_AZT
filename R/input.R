...
#Safa : discount_cost = 0.015, discount_qaly = 0.015
...
get_input <- function(age0 = 40,
  ...
  #Safa : new input
  input_help$medication$AZT_effect <- "Rate reduction in exacerbations only due to azithromycin"
  input$medication$AZT_effect <- 0.27
  input_ref$medication$AZT_effect <- "based on MACRO study"

  input_help$medication$is_AZT_group <- "Flag to determine we are running the code for the treatment group or the placebo group"
  input$medication$is_AZT_group <- 0
  input_ref$medication$is_AZT_group <- ""
...
#                                             ICS_LAMA_LABA_SABA=log((1-0.34)^input$medication$medication_adherence)))
  ...
  #Safa : Adverse Events - for Azithromycin
  ...
  input$manual$explicit_mortality_by_age_sex<-cbind(
    ...
    # Safa: Amin recalibrated mortality after exacerbation rate equation was recalibrated:
